...
const StatCard = ({ title, value, icon: Icon, color, trend }) => (
    <div className="bg-white p-8 rounded-none border border-slate-100 hover:border-red-700 transition-all duration-500 group">
...
const AdminDashboard = () => {
    ...
    const cards = [
        { title: 'Total Articles', value: stats.totalArticles, icon: FileEdit, color: 'bg-indigo-50 text-indigo-700', trend: 12 },
        { title: 'Global Views', value: stats.totalViews.toLocaleString(), icon: Eye, color: 'bg-emerald-50 text-emerald-700', trend: 24 },
        { title: 'Engagement (Likes)', value: stats.totalLikes.toLocaleString(), icon: Heart, color: 'bg-red-50 text-red-700', trend: 8 },
        { title: 'Approved Comments', value: stats.totalComments, icon: MessageSquare, color: 'bg-amber-50 text-amber-700', trend: 5 },
    ];
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                <thead className="border-b border-slate-50">
                                    <tr>
                                        <th className="pb-4 text-[9px] font-bold text-slate-400 uppercase tracking-widest">Headline</th>
                                        <th className="pb-4 text-[9px] font-bold text-slate-400 uppercase tracking-widest">Status</th>
                                        <th className="pb-4 text-[9px] font-bold text-slate-400 uppercase tracking-widest text-right">Date Created</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr key={article._id} className="group hover:bg-slate-50/50 transition-colors">
                                            <td className="py-6 pr-4">
                                                <Link to={`/admin/articles/edit/${article._id}`} className="block">
                                                    ...
                                                    <p className="text-[9px] text-slate-400 font-medium uppercase mt-1">ID: {article._id.slice(-8)}</p>
                                                </Link>
                                            </td>
                                            <td className="py-6">
                                                <span className={`px-2 py-0.5 text-[8px] font-black uppercase tracking-widest ${article.status === 'published'
                                            ...
                                            <td className="py-6 text-right">
                                                <p className="text-[9px] font-bold text-slate-900 tabular-nums">
                    ...
                    <div className="bg-white border border-slate-100 p-8 h-full">
                        ...
                        <div className="space-y-4">
                            <Link to="/admin/articles/new" className="flex items-center justify-between p-6 bg-slate-950 text-white hover:bg-red-700 transition-all duration-500 group">
                                <div className="flex items-center">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[8px] font-medium text-slate-500 group-hover:text-white/60 uppercase tracking-widest mt-1">Create new article</p>
                                    </div>
                                </div>
                            ...
                            <Link to="/admin/categories" className="flex items-center justify-between p-6 border border-slate-100 hover:border-red-700 transition-all duration-500 group">
                                <div className="flex items-center">
                                    <Layers size={20} className="mr-4 text-slate-300 group-hover:text-red-700" />
                                    <div>
                                        <p className="text-[10px] font-black text-slate-900 uppercase tracking-widest">Section Manager</p>
                                        <p className="text-[8px] font-medium text-slate-400 uppercase tracking-widest mt-1">Manage categories</p>
                                    </div>
                                </div>
                                <ChevronRight size={16} />
                            </Link>
                            ...
                            <div className="mt-12 pt-8 border-t border-slate-50">
                                ...
                                <div className="p-4 bg-slate-50">
                                    ...
                                    <p className="text-[9px] text-slate-500 uppercase tracking-wide leading-relaxed">
