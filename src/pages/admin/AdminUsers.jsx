    ...
    return (
        <div className="max-w-7xl mx-auto pb-20">
            ...
            <div className="bg-white border border-slate-100 p-4 mb-8 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
                ...
                <div className="flex bg-slate-50 p-1 w-full md:w-auto overflow-x-auto hidden-scrollbar">
                    {['', 'admin', 'user'].map((role) => (
                        <button
                            key={role}
                            onClick={() => setRoleFilter(role)}
                            className={`flex-1 px-4 py-2 text-xs font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap ${roleFilter === role
                            ...
                            {loading ? (
                                ...
                                filteredUsers.map((u) => (
                                    <tr key={u._id} className="group hover:bg-slate-50/50 transition-all duration-300">
                                        ...
                                        <td className="py-6 px-8">
                                            <div className={`flex items-center ${u._id === currentUser?._id && 'justify-center'} space-x-3`}>
                                                <RoleBadge role={u.role} />
                                                {u._id !== currentUser?._id && <select
                                        ...
                                        <td className="py-6 px-8 text-right">
                                            <button
                                                ...
                                                disabled={u._id === currentUser?._id}
                                                className="p-2.5 text-slate-500 cursor-pointer hover:text-red-700 hover:bg-red-50 transition-all disabled:hidden"
                                            >
                                                <Trash2 size={16} />
                                            </button>
                                        </td>
